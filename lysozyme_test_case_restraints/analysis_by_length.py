...
import convergence_test_bl as ct
...
auto_list = [timeseries.detect_equilibration(j[:max_t]) for j in dHdl]
dHdl_eq0 = [dHdl[j][auto_list[j][0]:max_t] for j in range(len(dHdl))]
...
#WATER STEP
watlist0 = glob.glob('./water-dcrg+vdw/la*/prod/*0[0-'+str(sys.argv[1])+'].out')
watlist0.sort()
...
wauto_list = [timeseries.detect_equilibration(j[:max_t]) for j in wdHdl]
wdHdl_eq0 = [wdHdl[j][wauto_list[j][0]:max_t] for j in range(len(wdHdl))]
...
for j in rlam_set:
    rdHdl_ssmp0.append(ct.analyze_rtr(j, int(sys.argv[1]), from_rtr=True)[:max_t])
...
df.to_csv('summary'+str(sys.argv[1])+'.dat', sep='\t', index=False)
