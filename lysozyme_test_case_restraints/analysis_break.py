...
import convergence_test_br as ct
...
auto_list = [timeseries.detect_equilibration(j[:max_time]) for j in dHdl]
dHdl_eq0 = [dHdl[j][auto_list[j][0]:max_time] for j in range(len(dHdl))]
...
wauto_list = [timeseries.detect_equilibration(j[:max_time]) for j in wdHdl]
wdHdl_eq0 = [wdHdl[j][wauto_list[j][0]:max_time] for j in range(len(wdHdl))]
...
for j in rlam_set:
    rdHdl_ssmp0.append(ct.analyze_rtr(j, int(sys.argv[1]), from_rtr=True))
...
df.to_csv('summary_breakr'+str(sys.argv[1])+'.dat', sep='\t', index=False)
