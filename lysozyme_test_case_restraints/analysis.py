...
rvarfirst=[]
rtr_lams = rlam_set
data1=np.concatenate(rdHdl_ssmp0[:3])
boot_samples1 = [np.array(resample(data1, n_samples=len(data1))) for k in range(10000)]
boot_mean1=np.array([k.mean() for k in boot_samples1])
rmean0.append(boot_mean1.mean())
rvar0.append(boot_mean1.std()**2)
for j in range(len(rdHdl_ssmp0)-3):
    data1=rdHdl_ssmp0[j+3]
    boot_samples1 = [resample(data1, n_samples=len(data1)) for k in range(10000)]
    boot_mean1=np.array([k.mean() for k in boot_samples1])
    rmean0.append(boot_mean1.mean())
    rvar0.append(boot_mean1.std()**2)
