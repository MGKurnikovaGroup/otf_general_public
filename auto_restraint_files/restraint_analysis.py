...
def valid(res_loc, mol_loc):
    ...
    if math.pi/4 >= A_B_C or 3*math.pi/4 <= A_B_C:
        ...
        #print('a_A_B', a_A_B/math.pi)
        if math.pi/4 >= a_A_B or 3*math.pi/4 <= a_A_B:
            return False
        else:
            #print(mol_loc, res_loc)
...
def angle_deviation(vec):
    vec = np.array(vec)
    return float((vec-math.pi/2).mean())
...
def choose_neighbors(res_loc, res_loc_2, mol_atom_a, n_names, neighbors_neighbors_names, mol_atom_a_loc, n_locations, nn_locations):
    ...
    valid_res_id = []
    #print('mol_loc')
    #print([mol_atom_a_loc, n_locations, nn_locations])
    for i in range(len(n_names)):
        for j in range(len(neighbors_neighbors_names[i])):
            #print(len({mol_atom_a_loc, n_locations[i], nn_locations[i][j]}))
    ...
    if len(valid_locs) == 0:
        ...
        print(len(valid_atoms))
        print(len(valid_res_id))
        return valid_locs[index], valid_atoms[index], valid_res_id[index]



def centroid_search():
    ligand=open('complex-repres.pdb', 'r')
    start = False
    xs=[]
    ys=[]
    zs=[]
    ...
    atom_names = []
    distances = []
    with open('complex-repres.pdb', 'r') as l:
        start_finding = False
        for line in l:
...
def find_ca_atoms():
    ca_names = []
    ca_locs = []
    with open('complex-repres.pdb', 'r') as f:
        first_ter = False
        for line in f:
